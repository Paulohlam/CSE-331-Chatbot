...
export function substitute(words: string[], reps: Map<string, string>): void {
  ...
  // Inv: words[0..j-1] = substitute(words0[0..j-1], reps)
  while (j < leng) {
    const curWord = words[j];
    const replace = reps.get(curWord);
...
export function joinWords(words: ReadonlyArray<string>): string {
  ...
  let j = 0;
  let parts: string[] = [words[0]];
  // join(parts) = join-words(words[0 .. j − 1])
